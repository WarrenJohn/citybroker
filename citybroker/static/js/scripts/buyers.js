...
function validate_radios(radios, valid){
    for (i = 0; i < radios.length; i++){
        if (radios[i].checked){
            radios.forEach(radio => radio.style = "");
            valid.push(true);
            return true;
        }
    }
    radios.forEach(radio => radio.style = "outline: 1px solid red");
    valid.push(false);
    return false;
}
...
function form_valid(form){
    ...
    for (let i of inputs){
        let node_list = document.getElementsByName(i.name);
        if (i.type === "radio"){
            if (last !== i.name){
                node_array.push(node_list);
                last = i.name;
            }
        }
    }
    node_array.forEach(a => validate_radios(a, is_valid) );
